...
class EmotionRecognition:
  ...
  def __init__(self):
    ...
    # self.model = None (added for clarity ~ Bryan)
    # self.lr =lr
  def build_network(self):
    ...
    img_prep.add_featurewise_zero_center(per_channel=True) # ORIGINAL
    img_prep.add_featurewise_stdnorm(per_channel=True) # ORIGINAL
    #img_prep.add_featurewise_zero_center(mean = 0) # Dana
    #img_prep.add_featurewise_stdnorm(std = 1) # Dana
    
    print('[+] Building CNN')
    self.network = input_data(shape = [None, SIZE_FACE, SIZE_FACE, 1], data_preprocessing=img_prep, data_augmentation=img_aug, dtype = 'float32')
    ...
    self.model.load(join(SAVE_DIRECTORY, SAVE_MODEL_FILENAME_TF), weights_only=True) # Dana :: this was commented out in their model
  ...
  def load_model(self):
    print(join(SAVE_DIRECTORY, SAVE_MODEL_FILENAME_TF +'.index'))
